                    ...
                    UWORD Color_Foreground, UWORD Color_Background)
{
    ...
    Paint_DrawChar(Xstart + Dx  + Dx - 1   , Ystart, ':'                    , Font, Color_Background, Color_Foreground);
...
void Paint_Color_Windows(uint8_t top_left_x, uint8_t top_left_y, uint8_t bot_right_x, uint8_t bot_right_y,uint16_t color,uint16_t dc)
{
    // 绘制上边框
    for (uint8_t i = top_left_x; i < bot_right_x; i++) {
        Paint_SetPixel(i, top_left_y, color);
				color += dc;
    }
    
    // 绘制右边框
    for (uint8_t j = top_left_y; j < bot_right_y; j++) {
        Paint_SetPixel(bot_right_x - 1, j, color);
				color += dc;
    }
    
    // 绘制下边框
    if (bot_right_y - 1 > top_left_y) { // 确保不重复绘制上边框
        for (uint8_t i = top_left_x; i < bot_right_x; i++) {
            Paint_SetPixel(i, bot_right_y - 1, color);
						color += dc;
        }
    }
    
    // 绘制左边框
    if (bot_right_x - 1 > top_left_x) { // 确保不重复绘制右边框
        for (uint8_t j = top_left_y; j < bot_right_y; j++) {
            Paint_SetPixel(top_left_x, j, color);
						color += dc;
        }
    }
}
...
*/
void Clock_init(uint16_t year,uint8_t month,uint8_t day,uint8_t hour,uint8_t min,uint8_t sec)
{
	sPaint_time.Year = year;
	sPaint_time.Month = month;
	sPaint_time.Day = day;
	sPaint_time.Hour = hour;
	sPaint_time.Min = min;
	sPaint_time.Sec = sec;
}
void Paint_System_Start(void)
{
	Paint_DrawLine(20, 11, 20, 31, Wheel(1), DOT_PIXEL_1X1, LINE_STYLE_SOLID);
	Paint_DrawLine(10, 21, 30, 21, Wheel(31), DOT_PIXEL_1X1, LINE_STYLE_SOLID);
	...
	Paint_DrawString_EN(5, 45, "world", &Font16, MAGENTA, BLACK);
}
void Paint_ALL_RGBWheel(int time_us){
	int count =0;
	Paint_Clear(BLACK);
	for(uint8_t i=0;i<255;i++){
		Paint_Clear(BLACK);
		for(uint8_t x=0;x<RGB_Matrix.width;x++){for(uint8_t y=0;y<RGB_Matrix.height;y++){Paint_SetPixel(x,y,Wheel(count));}}
		count++;
		HAL_Delay(time_us);
	}
}
void Paint_Clock(void)
{
	sPaint_time.Sec++;
	if(sPaint_time.Sec==60){sPaint_time.Sec=0;sPaint_time.Min++;}
	if(sPaint_time.Min==60){sPaint_time.Min=0;sPaint_time.Hour++;}
	if(sPaint_time.Hour==24){sPaint_time.Hour=0;sPaint_time.Day++;}
	Paint_Color_Windows(0,0,64,64,0,500);
	Paint_DrawString_EN(5, 5, "Varlike", &Font12, BLACK, YELLOW);
	Paint_DrawTime(5, 20,&sPaint_time, &Font12,BLACK,YELLOW);
}